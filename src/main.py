...
def check_missing_frames(directory):
    ...
    for filename in filenames[1:]:
        ...
        wid, hgt = img.size
        if specs["height"] != wid or specs["width"] != hgt: return [False,
                                                                    f"Incorrect size in {filename}: should be {specs['height']}x{specs['width']} while being {wid}x{hgt}"]
...
def add_part():
    ...
    while True:
        print(
            " - png: a series of PNG images. They must be named number by number (0000.png, 0001.png, 0002.png, ...)" + Fore.RESET)
        print(
            " - ffmpeg: Files will be decoded from ffmpeg to png and audio format. You can specify whatever file as long as FFmpeg supports it.")
    ...
    if med == "ffmpeg":
        part["ffmpeg"] = True
        if tk:
            p = crossfiledialog.open_file(title="Open a video file")
        else:
            p = prinp(QUES, "Please provide a path to the video:", "")
        part["path"] = [p, p]
    else:
        part["ffmpeg"] = False
        if tk:
            p = crossfiledialog.choose_folder("Select folder with pictures")
        else:
            p = prinp(QUES, "Please provide a path to the folder:", "")
        ...
        if prinp("QUES", "Would you also like to include WAV for this part? (y/n)", "n").lower() == "y":
            if tk:
                a = crossfiledialog.open_file(filter=["*.wav"])
            else:
                p = prinp(QUES, "Please provide a path to the audio:", "")
            part["path"] = [p, a]
        else:
...
try:
    ...
    if yea.lower() == "y":
        animations.append(add_part())
    if tk:
        loc = crossfiledialog.save_file(title="Save ZIP as")
    else:
        loc = prinp(QUES, "Enter location of ZIP", "")
    if not loc.endswith(".zip"):
        loc = loc + ".zip"
    ...
    prform(INFO, "It was saved under " + loc)
